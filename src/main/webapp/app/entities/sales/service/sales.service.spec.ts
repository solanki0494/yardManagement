import { TestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';

import { ISales } from '../sales.model';
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../sales.test-samples';

import { SalesService, RestSales } from './sales.service';

const requireRestSample: RestSales = {
  ...sampleWithRequiredData,
  saleTime: sampleWithRequiredData.saleTime?.toJSON(),
};

describe('Sales Service', () => {
  let service: SalesService;
  let httpMock: HttpTestingController;
  let expectedResult: ISales | ISales[] | boolean | null;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
    });
    expectedResult = null;
    service = TestBed.inject(SalesService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  describe('Service methods', () => {
    it('should find an element', () => {
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.find(123).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'GET' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should create a Sales', () => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const sales = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(sales).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'POST' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should update a Sales', () => {
      const sales = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(sales).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'PUT' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should partial update a Sales', () => {
      const patchObject = { ...sampleWithPartialData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.partialUpdate(patchObject).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'PATCH' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should return a list of Sales', () => {
      const returnedFromService = { ...requireRestSample };

      const expected = { ...sampleWithRequiredData };

      service.query().subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'GET' });
      req.flush([returnedFromService]);
      httpMock.verify();
      expect(expectedResult).toMatchObject([expected]);
    });

    it('should delete a Sales', () => {
      const expected = true;

      service.delete(123).subscribe(resp => (expectedResult = resp.ok));

      const req = httpMock.expectOne({ method: 'DELETE' });
      req.flush({ status: 200 });
      expect(expectedResult).toBe(expected);
    });

    describe('addSalesToCollectionIfMissing', () => {
      it('should add a Sales to an empty array', () => {
        const sales: ISales = sampleWithRequiredData;
        expectedResult = service.addSalesToCollectionIfMissing([], sales);
        expect(expectedResult).toHaveLength(1);
        expect(expectedResult).toContain(sales);
      });

      it('should not add a Sales to an array that contains it', () => {
        const sales: ISales = sampleWithRequiredData;
        const salesCollection: ISales[] = [
          {
            ...sales,
          },
          sampleWithPartialData,
        ];
        expectedResult = service.addSalesToCollectionIfMissing(salesCollection, sales);
        expect(expectedResult).toHaveLength(2);
      });

      it("should add a Sales to an array that doesn't contain it", () => {
        const sales: ISales = sampleWithRequiredData;
        const salesCollection: ISales[] = [sampleWithPartialData];
        expectedResult = service.addSalesToCollectionIfMissing(salesCollection, sales);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(sales);
      });

      it('should add only unique Sales to an array', () => {
        const salesArray: ISales[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const salesCollection: ISales[] = [sampleWithRequiredData];
        expectedResult = service.addSalesToCollectionIfMissing(salesCollection, ...salesArray);
        expect(expectedResult).toHaveLength(3);
      });

      it('should accept varargs', () => {
        const sales: ISales = sampleWithRequiredData;
        const sales2: ISales = sampleWithPartialData;
        expectedResult = service.addSalesToCollectionIfMissing([], sales, sales2);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(sales);
        expect(expectedResult).toContain(sales2);
      });

      it('should accept null and undefined values', () => {
        const sales: ISales = sampleWithRequiredData;
        expectedResult = service.addSalesToCollectionIfMissing([], null, sales, undefined);
        expect(expectedResult).toHaveLength(1);
        expect(expectedResult).toContain(sales);
      });

      it('should return initial array if no Sales is added', () => {
        const salesCollection: ISales[] = [sampleWithRequiredData];
        expectedResult = service.addSalesToCollectionIfMissing(salesCollection, undefined, null);
        expect(expectedResult).toEqual(salesCollection);
      });
    });

    describe('compareSales', () => {
      it('Should return true if both entities are null', () => {
        const entity1 = null;
        const entity2 = null;

        const compareResult = service.compareSales(entity1, entity2);

        expect(compareResult).toEqual(true);
      });

      it('Should return false if one entity is null', () => {
        const entity1 = { id: 123 };
        const entity2 = null;

        const compareResult1 = service.compareSales(entity1, entity2);
        const compareResult2 = service.compareSales(entity2, entity1);

        expect(compareResult1).toEqual(false);
        expect(compareResult2).toEqual(false);
      });

      it('Should return false if primaryKey differs', () => {
        const entity1 = { id: 123 };
        const entity2 = { id: 456 };

        const compareResult1 = service.compareSales(entity1, entity2);
        const compareResult2 = service.compareSales(entity2, entity1);

        expect(compareResult1).toEqual(false);
        expect(compareResult2).toEqual(false);
      });

      it('Should return false if primaryKey matches', () => {
        const entity1 = { id: 123 };
        const entity2 = { id: 123 };

        const compareResult1 = service.compareSales(entity1, entity2);
        const compareResult2 = service.compareSales(entity2, entity1);

        expect(compareResult1).toEqual(true);
        expect(compareResult2).toEqual(true);
      });
    });
  });

  afterEach(() => {
    httpMock.verify();
  });
});
