...
/**
 * REST controller for managing {@link com.swisui.yard.domain.Invoice}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class InvoiceResource {
    ...
    public ResponseEntity<Invoice> partialUpdateInvoice(
        ...
        Optional<Invoice> result = invoiceRepository
            ...
            .map(existingInvoice -> {
                ...
                if (invoice.getCompanyInvoiceNumber() != null) {
                    existingInvoice.setCompanyInvoiceNumber(invoice.getCompanyInvoiceNumber());
                }
    ...
    public List<Invoice> getAllInvoices(@RequestParam(required = false) String filter) {
        if ("purchase-is-null".equals(filter)) {
            log.debug("REST request to get all Invoices where purchase is null");
            return StreamSupport
                .stream(invoiceRepository.findAll().spliterator(), false)
                .filter(invoice -> invoice.getPurchase() == null)
